...
| 出入库记录 | 入库/出库单据，经办人自动取登录身份；出库超结存拦截、越仓操作拦截，保存联动结存与仓房状态；支持**更正**（按新旧差额联动）与**红冲作废**（原单留痕、反向冲销） |
...
| `POST /api/stock-records/{id}/void/` | 单据红冲作废（必填原因，反向冲销结存，原单保留） |
| `GET/POST/PUT /api/stock-records/` | 出入库单据（PUT/PATCH 为更正，按差额联动；不允许物理删除） |
...
- 出库数量超过批次结存时后端校验拒绝
- **单据更正**：未被后续业务锁定的普通出入库单可直接更正（方向、批次、数量、单价、时间等），系统按「新单影响 − 原单影响」的差额联动批次结存，换批次时同步调整新旧两个批次，原经办人保留、更正动作单独留痕
- **红冲作废**：单据不物理删除，置为已作废并反向冲销已联动的结存与仓房状态，必须填写作废原因，作废人/时间随原单永久可查；作废后不可再次更正或作废
- **不能随意改动的边界**：① 更正/作废后批次结存会为负的拒绝；② 仓房处于施药中/密闭中/通风散气的熏蒸作业期间，该仓所有出入库新增、更正、作废一律锁定；③ 单据时间已被一张「已调账」盘点覆盖（≤盘点日）时锁定，调整这类历史差异只能新建盘点单；④ 盘盈/盘亏调账单由盘点流程生成，不允许手工建、改、作废
...
- 盘点采用「保管员分仓录入实盘 → 库管主任统一调账」的操作与审核分离模式；盘点中（counting）允许各保管员反复补录漏盘仓房明细
- **盘点防重**：只有「盘点中」状态可以调账，已调账单重复提交 finish 一律拒绝（400），并对盘点单行加锁防止双击/并发生成第二遍调整流水；表头与日期仅主任/管理员可改，保管员只能维护自己仓房的实盘数与差异原因
